...
*
* FILENAME: sgemm2_L2_B64_U64_swi.cl		DESIGN REF: hydra00
*
	  ...
	  for (int i2 = 0; i2 < N; i2 += tile2)
	  {
	   for (int k1 = k2; k1 < k2 + tile2; k1 += tile1)
	   {
		...
		for (int k = k1; k < k1 + tile1; ++k)
		{
 		...
 		//WARING - This (#pragma unroll) will perform poorly due to dynamic interations
 		//Typically Unrolling on FPGA requires fixed loops to calculate
 		//loop unroll cycles properly
 		
 		//This "should" pipeline TILE_SIZE replication units of the
 		//following calculation (see WARNING above).               
